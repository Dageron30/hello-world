...
cache->blocks[i] = (CacheBlock*)malloc(cache->num_blocks_per_set *
sizeof(CacheBlock));
...
if (cache->blocks[set_index][i].valid && cache->blocks[set_index][i].tag ==
tag) {
block_index = i;
break;
}
...
// Initialize cache
Cache* cache = initCache(cache_size, block_size, associativity, true, true,
true);
...
float hit_rate = runCacheSimulation(cache_sizes[0], block_sizes[i],
associativities[0]);
...
float hit_rate = runCacheSimulation(cache_sizes[0], block_sizes[0],
associativities[i]);
...
/*
Add your answers here for questions:
- Explain the relationship between block size and hit rate. Support your answer by
results (hit rate vs
block size) you obtained using your simulator.
- Explain the relationship between associativity and hit rate. Support your answer
by results you
obtained using your simulator.
*/